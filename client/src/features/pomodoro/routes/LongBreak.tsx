...
import { POMODORO_PATH } from ".";
import { LONG_BREAK_TAB, POMODORO_BASE_PATH } from "..";
import {
  NavRightContent,
  PomodoroControls,
  PomodoroTabs,
...
const DEFAULT_LONG_BREAK_DURATION = 15; // in minutes

export const LongBreak: React.FunctionComponent = () => {
  ...
  useEffect(() => {
    setActiveTabId(LONG_BREAK_TAB);
  }, [setActiveTabId]);

  useEffect(() => {
    if (!!userPreferences) {
      setDuration(convertMinutesToMilliseconds(userPreferences.pomodoroLongBreakDuration));
    }
    ...
    >
      ...
      <CircleProgressBar
        ...
        max={convertMinutesToMilliseconds(
          userPreferences?.pomodoroLongBreakDuration || DEFAULT_LONG_BREAK_DURATION
        )}
      />
      <PomodoroControls
